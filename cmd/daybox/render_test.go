...
// must produce byte-identical output to the bash render_user_data, across
// the cases the awk gets subtly right — @REPO:/@FILE: includes with indent
// preservation, @SSH_KEYS@ list items, @PROFILE_SEED@ with TOML comment
// stripping that is multiline-string-aware (a #-line inside a """ block is
// content and survives; a #-line outside is stripped), the first-line
// shebang kept, and all @PLACEHOLDER@ substitutions.
//
// The golden file is generated by scripts/_golden-render.sh (the REAL bash
// renderer, sandboxed HOME). If it's missing, the test is skipped — run:
//   bash scripts/_golden-render.sh > cmd/daybox/testdata/render-golden.yaml
func TestRenderMatchesBashGolden(t *testing.T) {
	goldenPath := filepath.Join("testdata", "render-golden.yaml")
	want, err := os.ReadFile(goldenPath)
	if err != nil {
		t.Skipf("no golden file at %s — regenerate with: bash scripts/_golden-render.sh > %s", goldenPath, goldenPath)
	}
